<<<<<<< HEAD
# mongodb_connection.py - COMPLETE WITH ALL METHODS
...
class MongoDBConnection:
    ...
    def search_product_by_barcode(self, barcode):
        """Search for a product in MongoDB by barcode"""
        try:
            products = self.get_collection('products')
            ...
            product = products.find_one({
                "$or": [
                    ...
                    {"barcode": barcode},
                    {"barcodes": {"$in": [barcode_clean, barcode]}}
                ]
            })
    ...
    def add_product_to_database(self, product_data):
        ...
        try:
            ...
            existing = self.search_product_by_barcode(product_data['barcode'])
            if existing:
                return {"success": False, "message": "Product already exists in database"}
            ...
            product_doc = {
                ...
                'is_b12': product_data.get('is_b12', False),
                'quantity': product_data.get('quantity', 'Not specified'),
                'categories': product_data.get('categories', ''),
                'image_url': product_data.get('image_url', ''),
                'allergens': product_data.get('allergens', ''),
                'nutrition': product_data.get('nutrition', {}),
                'source': product_data.get('source', 'Manual Entry'),
                'created_at': datetime.now(),
                'updated_at': datetime.now()
            }
    ...
    def get_all_products(self, limit=100):
        ...
        try:
            products = self.get_collection('products')
            cursor = products.find().sort('created_at', -1).limit(limit)
            results = list(cursor)
    ...
    def get_b12_products(self, limit=50):
        """Get all B12 products from database"""
        try:
            products = self.get_collection('products')
            cursor = products.find({"is_b12": True}).sort('created_at', -1).limit(limit)
            results = list(cursor)
            ...
            print(f"❌ Error retrieving B12 products: {e}")
            return []
    
    def save_scanned_product(self, user_id, product_data):
        """Save a scanned product to user's history"""
        try:
            scanned = self.get_collection('scanned_products')
            ...
            scan_doc = {
                ...
                'is_b12': product_data.get('is_b12', False),
                'source': product_data.get('source', 'API'),
                'scanned_at': datetime.now()
            }
            
            result = scanned.insert_one(scan_doc)
            ...
            print(f"❌ Error saving scanned product: {e}")
            return {"success": False, "message": f"Error: {str(e)}"}
...
def migrate_temp_logs_to_mongodb(self, user_id, temp_logs):
    """
    Migrate temporary logs to MongoDB after user login
    """
    try:
        migrated_count = 0
        ...
        for log_entry in temp_logs:
            ...
            log_entry['migrated_at'] = datetime.now().isoformat()
            
            # Insert into appropriate collection
            if 'symptom' in log_entry:
                # Daily tracker logs
                self.db.daily_tracker.insert_one(log_entry)
            elif 'type' in log_entry and log_entry['type'] == 'supplement':
                # Supplement logs
                self.db.supplement_logs.insert_one(log_entry)
            elif 'type' in log_entry and log_entry['type'] == 'food':
                # Food logs
                self.db.food_logs.insert_one(log_entry)
            else:
                # General logs
        ...
        return {
            ...
            'message': f'Migrated {migrated_count} logs successfully'
        }
...
# Add these to your mongodb_connection.py
...
def save_all_tracker_data(self, user_id, tracker_data):
    ...
    try:
        ...
        if tracker_data:
            self.db.tracker_entries.insert_many(tracker_data)
        
        return True
    except Exception as e:
        print(f"Error saving all tracker data: {e}")
        return False
# Add these methods to your MongoDBConnection class
...
>>>>>>> 7c36875a3473a43fa4acbe3693b875086aab96d7
